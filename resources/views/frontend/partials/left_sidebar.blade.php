...
<div class="card card-body mt-5 left-sidebar" style="background-color:#32272e">
    <img style="margin-bottom: 20px;" src="{{ asset('public/images/users/'.Auth::guard('web')->user()->image) }}" class="rounded-circle mx-auto d-block" alt="" width="150" height="100">
    <h5 class="ml-2">{{ Auth::guard('web')->user()->name }}</h5>
    <h5 class="ml-2">{{ Auth::guard('web')->user()->account_role }}</h5>
    <a href="{{ route('user.profile') }}" class="list-group-item">
      <span class="float-left item">My Profile</span> 
    </a>
</div>
@endif
